...
const CoreValues = () => {
  ...
  const coreStrengths = [
    {
      icon: <BeakerIcon className="h-12 w-12 text-primary mb-4" />,
      title: '독보적인 기술력',
    ...
    {
      icon: <GlobeAltIcon className="h-12 w-12 text-primary mb-4" />,
      title: '환경 보호 기여',
      description: '대기 오염 물질 저감 및 자원 재활용을 통해 깨끗한 환경을 만들어갑니다.',
    },
    {
      icon: <LifebuoyIcon className="h-12 w-12 text-primary mb-4" />,
      title: '고객 맞춤형 지원',
          ...
          {coreStrengths.map((strength, index) => (
            ...
            >
              <div className="bg-white rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-500 border border-gray-100 hover:border-primary/20 transform hover:-translate-y-2">
                {/* 상단 아이콘 영역 */}
                <div className="mb-6">
                  <div className="w-16 h-16 bg-gradient-to-br from-primary to-accent rounded-xl flex items-center justify-center mx-auto group-hover:scale-110 transition-transform duration-300">
